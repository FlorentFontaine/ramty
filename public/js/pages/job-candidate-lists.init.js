...
var staff = {
    init: function (allcandidateList) {
            ...
            for (var i = (page - 1) * itemsPerPage; i < (page * itemsPerPage) && i < datas.length; i++) {
                ...
                if (datas[i]) {
                    ...
                    var isUserProfile = datas[i].userImg ? '<img src="' + datas[i].userImg + '" alt="" class="member-img img-fluid d-block rounded" />'
                        : '<div class="avatar-title border bg-light text-primary rounded text-uppercase fs-16">' + datas[i].fullName + '</div>';
                    ...
                    <div class="card-body">\
                        <div class="d-lg-flex align-items-center">\
                            ...
                            <div class="ms-lg-3 my-3 my-lg-0">\
                                <a href="pages-profile"><h5 class="fs-16 mb-2">'+ datas[i].fullName + '</h5></a>\
                                <p class="text-muted mb-0">'+ datas[i].email + '</p>\
                            </div>\
                            <div class="d-flex gap-4 mt-0 text-muted mx-auto">\
                                <div><i class="ri-map-pin-2-line text-primary me-1 align-bottom"></i> '+ datas[i].address + '</div>\
                                <div><i class="ri-time-line text-primary me-1 align-bottom"></i> '+ datas[i].createdAt + '</div>\
                            </div>\
                            <div class="d-flex flex-wrap gap-2 align-items-center mx-auto my-3 my-lg-0">\
                                <div class="badge text-bg-success">\
                                    <i class="mdi mdi-star me-1"></i>'+ datas[i].type + '\
                                </div>\
                                <div class="text-muted">'+ datas[i].nombreEvents + '</div>\
                            </div>\
                            <div>\
        ...
        searchElementList.addEventListener("keyup", function () {
            ...
            function filterItems(arr, query) {
                return arr.filter(function (el) {
                    return el.fullName.toLowerCase().indexOf(query.toLowerCase()) !== -1 || el.email.toLowerCase().indexOf(query.toLowerCase()) !== -1
                })
            }
